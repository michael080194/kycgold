...
function startScan() {
    // alert("04:Starting scan for devices...", "status");
    foundDevices = [];
...
function retrieveConnectedSuccess(result) {
    alert("retrieveConnectedSuccess()");
    alert(result);
...
function startScanSuccess(result) {
    // alert("05:startScanSuccess(" + result.status + ")");
    if (result.status === "scanStarted") {
        // alert("06:Scanning for devices (will continue to scan until you select a device)...", "status");
...
function connect(address) {
 alert('Connecting to device: ' + address + "...", "status");
...
function connectSuccess(result) {
    alert("_ XXXX _" + result.status);
    // return false;
    if (result.status === "connected") {
        getDeviceServices(result.address);
    }else if (result.status === "disconnected") {
        alert("Disconnected from device: " + result.address, "status");
    }
}

function getDeviceServices(address) {
    alert("Getting device services...", "status");
...
function discoverSuccess(result) {
    alert("Discover returned with status: " + result.status);
